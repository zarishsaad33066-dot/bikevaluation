...
>(
  (
    ...
    // We add a resize listener to check if we are on a mobile device.
    React.useEffect(() => {
      const handleResize = () => {
  ...
  (
    ...
    if (isMobile) {
      return (
        <div
          ...
          data-mobile="true"
          className={cn(
            "group peer fixed inset-y-0 z-50 hidden w-[--sidebar-width-mobile] bg-sidebar p-0 text-sidebar-foreground [&_[data-sidebar=sidebar]]:flex",
    ...
    return (
      <div
        ...
        data-collapsible={collapsible}
        className={cn(
          "group peer hidden md:flex h-full w-[--sidebar-width] shrink-0 bg-sidebar text-sidebar-foreground",
          state === "open" &&
            variant === "sidebar" &&
            "w-[--sidebar-width]",
          state === "closed" &&
...
>(({ className, showIcon = false, ...props }, ref) => {
  ...
  return (
    ...
    >
      {showIcon && (
        <div className="flex aspect-square w-4 h-4 rounded-sm bg-sidebar-accent" />
      )}
      <div
        className="flex-1 h-4 rounded-sm bg-sidebar-accent max-w-[--skeleton-width]"
        style={
          {
